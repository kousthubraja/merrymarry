...
function handleResponse (res) {
  ...
  var allProfiles = JSON.parse(fs.readFileSync('kr_profiles.json', 'utf8'));
  allProfiles = allProfiles.concat(profilesInPage);
  var json = JSON.stringify(allProfiles);
  fs.writeFile('kr_profiles.json', json, 'utf8', ()=>{});
}
...
async function run () {
  for (var start = 0; start <= 10000; start += 20) {
    await fetch ('https://mg.bharatmatrimony.com/search/matches/E5710033', {
      headers: {
